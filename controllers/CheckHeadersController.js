  ...
  return new Promise((resolve, reject) => {
    ...
    const socket = tls.connect(options, () => {
      ...
      resolve({
        ...
        sniSupported: options.servername ? 'SNI supported' : 'SNI not supported',
      });
...
const checkMixedContentAndIframes = async (url) => {
  try {
    ...
    const mixedContent = html.includes('http://') ? 'Mixed content detected' : 'None';
...
const CheckHeaders = async (req, res) => {
  ...
  try {
    ...
    if (error.code === 'ECONNRESET') {
      errorDetails = ': Possible causes include the server blocking the request or firewall issues (such as IP whitelisting).';
    } else if (error.code === 'ENOTFOUND') {
      errorDetails = ': Hostname could not be found. The URL might be invalid or blocked by a firewall (such as IP whitelisting).';
    } else if (error.message.includes('timeout')) {
      errorDetails = ': Request timed out. The server may not be reachable, or the request was blocked by a firewall (such as IP whitelisting).';
    } else {
